...
With that the final column of our prototype has been finished! The final resul:
...
Because the styles for the active states of both the `equipped items` and the `grid items` are almost identical, we can laverage <Highlight>SCSS `@mixin`</Highlight>.
