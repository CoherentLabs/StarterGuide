...
## Strengthening Borders and Outlines
...
:root.theme-high-contrast .menu-button {
    ...
    outline: none; /* Remove the default outline since the border handles it */
}
...
The focus indicator (yellow outline on button focus) is critical in high contrast mode. Players who rely on keyboard or gamepad navigation need to see exactly which element is focused. In high contrast mode, the focus indicator should be more prominent than in the default theme, not less.
...
High contrast mode and font scaling are independent systems that should compose correctly. Both operate through CSS custom properties and root classes. A player can have `theme-high-contrast` and `scale-large` active simultaneously without either breaking the other:
...
- [ ] The focused element has a clearly visible outline (3px+ yellow outline recommended).
