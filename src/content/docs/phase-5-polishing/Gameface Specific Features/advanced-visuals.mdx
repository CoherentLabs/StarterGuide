...
Both backdrop filter and surface partitioning involve the engine integration layer. The next article covers blending the UI with the game world at the rendering pipeline level, including alpha compositing modes and how Gameface's transparency interacts with the scene underneath.
