...
With E2E tests catching regressions in interactions, bindings, and navigation, your UI has a reliable safety net for iterating quickly. The Gameface E2E documentation at [frontend-tools.coherent-labs.com/e2e](https://frontend-tools.coherent-labs.com/e2e/getting-started/) covers the full API reference for every `gf` and `DOMElement` method, along with additional examples in the npm package's `examples` folder.
